...
public class BookControllerIT extends AbstractBaseIT {
    ...
    @Test
    @WithAnonymousUser
    public void should_not_include_the_book_status_in_the_book_details_if_the_user_is_anonymous() throws Exception{
        mvc.perform(MockMvcRequestBuilders.get("/books/1").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.title", is("The Iliad")))
                ...
                .andExpect(jsonPath("$.averageRating", is(5.0)))
                .andExpect(jsonPath("$.reviews[0].highlight").doesNotExist())
                .andExpect(jsonPath("$.reviews[0].rating", is(5)))
                .andExpect(jsonPath("$.reviews[0].reviewer", is("Philip Ford")));
    }
    ...
    @WithUserDetails(value="jsmith@gmail.com", userDetailsServiceBeanName = "userDetailsServiceImpl")
    @DirtiesContext
    public void should_not_highlight_books_checked_out_by_someone_else() throws Exception{
        mvc.perform(MockMvcRequestBuilders.get("/books/1").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.title", is("The Iliad")))
